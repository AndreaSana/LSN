...
vector<Travel> New_generation(Random& rn, const vector<Travel>& travels, const string& namefile, const vector<double>& temp, const int& n_gen, const int& rank, const int& size)
{
    vector<Travel> new_generation;

    for(int i = 0; i < int(travels.size()); i++)
    {
        Travel son = travels[i];   // New individual
        son = Pair_permutation(rn, son);
        double A = min(1., exp(-(son.L_2() - travels[i].L_2()) / temp[i]));  	// Transition probability 
        // Reject new position
        if (rn.Rannyu() > A)
            son = travels[i];      // Restore previus travel
        
        // Shift of block of cities
        son = Shift(rn, son);
        A = min(1., exp(-(son.L_2() - travels[i].L_2()) / temp[i]));  	// Transition probability 
        // Reject new position
        if (rn.Rannyu() > A)
            son = travels[i];      // Restore previus travel
        ...
        new_generation.push_back(son);
    }

    for (int i = 1; i < int(new_generation.size()); i++)
    {
        if (n_gen % 2 == (i + int(new_generation.size()) * rank) % 2)
        {
            double A = min(1., exp(-(1. / temp[i - 1] - 1. / temp[i])*(new_generation[i].L_2() - new_generation[i - 1].L_2())));   // Transition probability
            
            if (rn.Rannyu() < A)
            {
                Travel travel = new_generation[i];
                new_generation[i] = new_generation[i - 1];
                new_generation[i - 1] = travel;
            }
        }
    }
    ...
    if(rank != 0 && (int(new_generation.size()) * rank) % 2 == n_gen % 2)
    {
        MPI_Send(&temp[0], 1, MPI_DOUBLE, rank - 1, tag_temp, MPI_COMM_WORLD);                                      // Send temperature
        L_rec = new_generation[0].L_2();
        MPI_Send(&L_rec, 1, MPI_DOUBLE, rank - 1, tag_L, MPI_COMM_WORLD);                                        // Send cost function
        MPI_Send(new_generation[0].Get_index_cities().data(), new_generation[0].Get_size(), MPI_INT, rank - 1, tag_index, MPI_COMM_WORLD);   // Send indexes of travel
    }

    MPI_Barrier(MPI_COMM_WORLD);    // Synchronize all cores

    if(rank != size - 1 && (int(new_generation.size()) * (rank + 1) - 1) % 2 != n_gen % 2)
    {
        ...
        vector<int> index_rec(new_generation[0].Get_size());
        MPI_Recv(index_rec.data(), new_generation[0].Get_size(), MPI_INT, rank + 1, tag_index, MPI_COMM_WORLD, &stat_3);       // Receive indexes of travel

        // Exchange of paths between processes
        double A = min(1., exp(-(1. / temp[new_generation.size() - 1] - 1. / temp_rec)*(L_rec - new_generation[new_generation.size() - 1].L_2())));   // Transition probability 
        if (rn.Rannyu() < A)
        {
            check_exchange = true;
            MPI_Send(new_generation[new_generation.size() - 1].Get_index_cities().data(), new_generation[new_generation.size() - 1].Get_size(), MPI_INT, rank + 1, tag_index, MPI_COMM_WORLD);   // Send indexes of travel    
            new_generation[new_generation.size() - 1].Set_index_travel(index_rec);  // Set new travel
        }       
    ...
    if(rank != 0 && (int(new_generation.size()) * rank) % 2 == n_gen % 2)
    {
        ...
        if(check_rec)
        {
            vector<int> index_rec(new_generation[0].Get_size());
            MPI_Recv(index_rec.data(), new_generation[0].Get_size(), MPI_INT, rank - 1, tag_index, MPI_COMM_WORLD, &stat_3);       // Receive indexes of travel
            new_generation[0].Set_index_travel(index_rec);  // Set new travel
        }
    }
