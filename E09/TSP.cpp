...
void TSP :: Evolution(const string& name)
{
    // Output file with names colums
    string namefile = "L_" + name + ".dat";	
	ofstream sout(namefile);
    sout << "Gen L" << endl;
    sout.close();
    string namefile2 = "L_half_" + name + ".dat";	
	ofstream sout2(namefile2);
    sout2 << "Gen L" << endl;
    sout2.close();

    _travels[0].Print_travel(_current_gen, name);   // Print best travel of generation
    ...
    {    
        ...
        _travels = New_generation(_rn, _travels, name);     // Create new generation
        _travels[0].Print_travel(_current_gen, name);       // Print best travel of new generation
    }
}

// Create n_gen generations
void TSP :: Evolution(const int& n_gen, const string& namefile) 
{
    for (int i = 0; i < n_gen; i++)     // Loop over generations
    {
        _current_gen++;                                       // Increase generation
       _travels = New_generation(_rn, _travels, namefile);    // Create new generation
    }

    // Update number of generations
    if (_current_gen > _n_generation)
        _n_generation = _current_gen;
}
