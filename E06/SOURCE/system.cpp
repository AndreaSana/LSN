...
// the ../INPUT/ directory
void System ::initialize(string path_input, string path_output, double temp)
{
  ...
  if (input.is_open()) 
    ...
    while (!input.eof()) 
      ...
      if (property == "POTENTIAL_ENERGY") 
        ...
        index_property++;
        _ptail = (32 * M_PI * _rho) / (3 * pow(_r_cut, 3)) * ( 1 / (3 * pow(_r_cut, 6)) - 0.5); // TO BE FIXED IN EXERCISE 7
      } 
...
void System ::write_configuration(string path_output) 
{
  ...
  {
    coutf.open(path_output + "CONFIG/config.spin");
    for (int i = 0; i < _npart; i++)
...
{
  ifstream cinf;
  cinf.open(path_input + "CONFIG/config.ising");
  if (cinf.is_open()) 
  {
    string comment;
    string particle;
